...
class ActivitiesController extends AbstractController
{
        ...
        foreach ($data['monitors'] as $monitorId) {
            ...
            if (!$monitor->getActivities()->isEmpty()) {
                foreach ($monitor->getActivities() as $monitorActivity) {
                    if ($monitorActivity->getDateStart() == $date) {
    ...
    public function update(int $id, Request $request, EntityManagerInterface $entityManager): JsonResponse
    {
        ...
        // Valido que la fecha es correcta y cumple con los requisitos establecidos.
        if (!empty($data['date_start'])) {
            
            $inputDate = \DateTime::createFromFormat('Y-m-d H:i:s', $data['date_start']);

            if (!$inputDate || $inputDate->format('H:i') != '09:00' && $inputDate->format('H:i') != '13:30' && $inputDate->format('H:i') != '17:30') {
                return $this->json(['error' => 'Fecha incorrecta, tiene que ser 09:00, 13:30 o 17:30.'], 400);
            }

            // Valido que la actividad dure 90 minutos
            $dateEnd = clone $inputDate;
            $dateEnd->modify('+90 minutes');

            if ($dateEnd->format('H:i') != '10:30' && $dateEnd->format('H:i') != '15:00' && $dateEnd->format('H:i') != '18:30') {
                return $this->json(['error' => 'La duración de la actividad debe ser de 90 minutos.'], 400);
            }

            // Si la fecha es correcta, se la asignamos a la actividad a modificar.
            $activity->setDateStart($inputDate);
            $activity->setDateEnd($dateEnd);
        }

        // Comprobamos que el tipo de actividad se ha proporcionado y que exista en la base de datos.
        if (!empty($data['activity_type_id'])) {
            $activityType = $entityManager->getRepository(ActivityType::class)->find($data['activity_type_id']);

            if (!$activityType) {
                return $this->json(['error' => 'El tipo de actividad proporcionado no existe en la base de datos.'], 404);
            }
            
            // Si el tipo de actividad es correcto, se lo asignamos a la actividad a modificar.
            $activity->setActivityType($activityType);
        }
        ...
        if (!empty($data['monitors'])) {
            ...
            foreach ($data['monitors'] as $monitorId) {
                ...
                if (!$monitor->getActivities()->isEmpty()) {
                    foreach ($monitor->getActivities() as $monitorActivity) {
                        if ($monitorActivity->getDateStart() == $inputDate) {
                            return $this->json(['error' => 'El monitor ya está asignado en ese horario.'], 400);
                        }
                    }
                }
